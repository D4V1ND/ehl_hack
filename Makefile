# Slice B (backend + packages) and Slice A (apps/erp). One entry point for both.
#
...
NODE := bash -lc 'cd apps/erp && { source "$$NVM_DIR/nvm.sh" 2>/dev/null && nvm use >/dev/null 2>&1; };
...
clean:
	rm -rf apps/erp/.next cases/CASE-00*/ backend/record/*.db
