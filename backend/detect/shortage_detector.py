"""B4 -- the shortage detector. The "no human in the loop" trigger.
...
Either one opens a case directory and writes the first event. What happens next
-- creating the Devin session and running the sourcing procedure -- is Slice D's
launcher; this module calls the hook it is given and stops there. That seam is
deliberate: Slice B decides *that* a case exists, Slice D decides what to do
about it.

Built last on purpose. It is thirty lines, and it is only worth switching on
once a human-triggered case runs green end to end.
"""
...
"""Slice D's session launcher. Given an incident, returns a session URL if it started one."""
