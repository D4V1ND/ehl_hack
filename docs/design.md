...
- **Messaging** — the linked Incident is an inline primary-colour mention in the first user message. The fixed bottom composer uses a spinner while a message submits and a square stop control while the agent streams. The current rehearsal appends local messages after 1.5 seconds.
