...
- **Sessions** — compact two-line mock Session links have no icon. A selected Session uses the muted background and foreground text. An unselected Session uses muted foreground text without a background. A completed Session uses bold foreground text in either state. Every selection opens the same fixture thread.
