...
- **Sessions** — compact mock Session links use the part label as their sole title and have no icon; the selected Session uses the accent background. Every selection opens the same fixture thread.
...
- **Messaging** — the linked Incident is an inline muted mention with foreground text and one accent pebble in the first user message. The fixed bottom composer uses a spinner while a message submits and a square stop control while the agent streams. The current rehearsal appends local messages after 1.5 seconds.
