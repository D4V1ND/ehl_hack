...
export function DecisionPanel({
  ...
  // Follow the agent when it re-prices, until the buyer picks for themselves.
  useEffect(() => {
    if (chosen === null && decision) setChosen(decision.recommended_strategy_id)
  }, [chosen, decision])
  ...
  return (
    ...
    >
      ...
      >
        {offered.map((option) => (
          <PlanCard
            ...
            onSelect={() => setChosen(option.strategy_id)}
          />
        ))}
      </div>
      ...
      <footer className="flex flex-wrap items-center gap-x-3 gap-y-1 border-t border-border bg-muted/30 px-4 py-2.5">
        ...
        {caseId ? (
          <a
            href={`/backend/cases/${encodeURIComponent(caseId)}/artifacts/decision.md`}
            target="_blank"
            rel="noreferrer"
