...
def test_price_break_is_a_step_function_not_a_curve(suppliers):
    ...
    assert unit_price(skf.price_breaks, 999, skf.contract_unit_price) == Decimal("1.95")
    assert unit_price(skf.price_breaks, 1000) == Decimal("2.10")
    assert unit_price(skf.price_breaks, 9999) == Decimal("2.10")
    assert unit_price(skf.price_breaks, 10000) == Decimal("1.88")
    assert unit_price(skf.price_breaks, 25000) == Decimal("1.72")


def test_below_the_smallest_tier_the_contract_price_is_the_honest_answer(suppliers):
    skf = suppliers["SUP-SKF"]
    assert unit_price(skf.price_breaks, 500, skf.contract_unit_price) == Decimal("1.95")
    assert unit_price([], 500) == Decimal("0")


def test_landed_cost_adds_freight_duty_and_carrying_by_hand(suppliers, part, profile):
    """4,000 pcs from SKF Nordic, by road.

    goods    4,000 x 2.10                              = 8,400.00
    ...
    cost = landed_cost(
        line=Line(supplier=skf, qty=4000, mode=FreightMode.ROAD, eta=arrival),
        part=part,
        profile=profile,
        needed_by=NEEDED_BY,
        daily_consumption=TAKE_RATE,
    )
    assert cost.goods_cost == Decimal("8400.00")
    ...
    assert cost.unit_effective > Decimal("2.10"), "landed is always above the sticker"
...
def test_duty_is_ad_valorem_on_goods_plus_freight(suppliers, part, profile):
    ...
    goods = Decimal("10000") * Decimal("1.15")
...
def test_every_component_is_decimal_not_float(suppliers, part, profile):
    skf = suppliers["SUP-SKF"]
    cost = landed_cost(
        line=Line(supplier=skf, qty=12000, mode=FreightMode.ROAD, eta=date(2026, 9, 1)),
        part=part,
        profile=profile,
        needed_by=NEEDED_BY,
        daily_consumption=TAKE_RATE,
...
def test_air_buys_days_and_charges_for_them(suppliers, part, profile):
    ...
    shenzhen = suppliers["SUP-SHZ"]
    by_sea = eta(supplier=shenzhen, profile=profile, mode=FreightMode.SEA, today=TODAY)
    by_air = eta(supplier=shenzhen, profile=profile, mode=FreightMode.AIR, today=TODAY)
    assert (by_sea - by_air).days == 29, "air reduces a 30-day route to one day"

    sea_cost = landed_cost(
        line=Line(supplier=shenzhen, qty=10000, mode=FreightMode.SEA, eta=by_sea),
        part=part, profile=profile, needed_by=NEEDED_BY, daily_consumption=TAKE_RATE,
    )
    air_cost = landed_cost(
        line=Line(supplier=shenzhen, qty=10000, mode=FreightMode.AIR, eta=by_air),
        part=part, profile=profile, needed_by=NEEDED_BY, daily_consumption=TAKE_RATE,
    )
