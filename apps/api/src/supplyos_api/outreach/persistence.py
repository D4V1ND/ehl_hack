"""Persist completed outreach quotes and their evidence to disk.

``OUTREACH_BUFFER`` is a transient working set that dies with the process, so
a call that cost real money and real time must also leave a durable trace.
Every completed live quote is written here as JSON, including the full ``raw``
CALL-E record and transcript turns: that evidence is worth more than the typed
fields alone.
