...
DB_PATH = REPO_ROOT / "apps" / "api" / "data" / "erp.db"
...
class SqliteERP:
    ...
    def __init__(self, db_path: Path | None = None) -> None:
        ...
        if not self.db_path.exists():
            raise DatabaseMissing(
                f"no database at {self.db_path}. Run `python run.py db` "
                "to build it from the seed YAML."
            )
    ...
    def get_company_profile(self) -> CompanyProfile:
        ...
        if row is None:
            raise DatabaseMissing("tabCompanyProfile is empty; rebuild with `python run.py db`")
