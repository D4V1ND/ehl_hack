...
The workflow is written against the `SystemOfRecord` interface, so if the YAML
and SQL backends ever disagree, configuration changes behavior. This walks
every read and asserts the answers are byte
