...
from supplyos_api.deps import settings, store
...
from supplyos_api.launch.devin import session_prompt, start_session
from supplyos_api.launch.incident import case_id_for, incident_for_part
from supplyos_api.launch.resolve import resolve_incident
from supplyos_api.outreach.calle import InvalidPhoneNumber, _load_supplier_phones
from supplyos_api.policy.rules import deliverable_qty
...
def client(tmp_path, monkeypatch):
    ...
    app.dependency_overrides[settings] = lambda: replace(
        get_settings(), live_calls_enabled=False, github_token=None, github_repo=None
    )
...
def test_the_session_starts_with_no_secrets_and_a_cap(records, monkeypatch):
    ...
    monkeypatch.setattr("supplyos_api.launch.devin.httpx.post", _post)
    incident = incident_for_part(records=records, part_id="PRT-6204", today=TODAY)
...
def test_the_ui_on_the_fallback_dev_port_is_allowed_by_default():
    ...
    from supplyos_api.settings import cors_origins_from_env
